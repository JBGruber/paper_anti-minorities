...
4. Is the statement referring to immigrants or ethnic minorities?

Return answers in the following format:
{\"1_lgbtq\": \"<Yes/No>\", \"2_party\": \"<Party Name or NA>\", \"3_evaluation\": \"<positive/neutral/negative or NA>\", \"4_immigration\": \"<Yes/No>\"}
"
...
make_query <- function(txt) {
  map(txt, function(t) {
    tribble(
      ...
      "assistant", "{\"1_lgbtq\": \"Yes\", \"2_party\": \"Labour\", \"3_evaluation\": \"positive\", \"4_immigration\": \"No\"}",
      ...
      "assistant",  "{\"1_lgbtq\": \"Yes\", \"2_party\": \"Conservatives\", \"3_evaluation\": \"positive\", \"4_immigration\": \"Yes\"}",
      "user", "The Green Party is focused on addressing transgender rights, climate change and economic inequality.",
      "assistant", "{\"1_lgbtq\": \"No\", \"2_party\": \"Green Party\", \"3_evaluation\": NA, \"4_immigration\": \"No\"}",
      "user", t
    )
  })
}
...
```{r}
Sys.setenv(OLLAMA_CONS = 1)
paragraphs_annotated <- paragraphs |> 
  mutate(
    query = make_query(txt = text),
    req = query(query, screen = FALSE,
                       model = "llama3.1:8b-instruct-q8_0", 
                       # server = "http://192.168.2.29:11434/", 
                       output = "httr2_request",
                       format = "json",
                       model_params = list(seed = 42, temperature = 0)),
    annotation = httr2::req_perform_sequential(req)
  )
```
...
paragraphs_annotated_tidy <- paragraphs_annotated |> 
  mutate(annotation = map_chr(annotation, function(resp) {
...
paragraphs_annotated_tidy |> 
  select(-req, -par_id, -word, -set, -query) |> 
  View("annotated")
```
